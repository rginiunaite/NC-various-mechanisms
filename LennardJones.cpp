...
//    double y_init = 120.0;
    ...
    double npow = 1.0; // Lennard-Jones type model powers
    double mpow = 2.0*npow; //  Lennard-Jones type model powers
    ...
    double search_param = 50; //radius to search for cell-cell interactions
    double f0 = 5.0;// strength of the force
...
//                                                            double(N)); // x=radius, uniformly in y
    ...
    // for Lennard-Jones
    std::normal_distribution<double> normalX(0,2); // mean 1 variance 1
...
//            x[0] = x[0] + Gamma(value)-Gamma_old(value);
            ...
            cout << "position before " << get<position>(particles)[i] << endl;
            get<position>(particles)[i] += vdouble2(Gamma(value)-Gamma_old(value),0);
            cout << "position after " << get<position>(particles)[i] << endl;
...
#ifdef HAVE_VTK
            vtkWriteGrid("GROWNEWCILParticles", t, particles.get_grid(true));
#endif




            ofstream output("GROWNEWCILMatrix" + to_string(int(t)) + ".csv");
